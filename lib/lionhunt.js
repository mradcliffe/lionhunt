...
function lionhunt(options) {
    ...
    this._getTasksForTeam = function(teams) {
        var fields = [
            "ID", "estimate", "name", "description", "status", "priority", "referenceNumber",
            ...
            "updates:message", "updates:refObjCode", "updates:threadID", "updates:subObjID",
            "updates:subMessage", "statusEquatesWith", "project:name", "documents:downloadURL",
            ...
            _.forEach(task.parameterValues, function(value, key) {
                ...
                if (map !== false) {
                    issue.customFieldValues.push({
                        fieldName: map.to,
                        fieldType: map.type,
                        value: map.type === "com.atlassian.jira.plugin.system.customfieldtypes:textarea" ? self._formatToJira(value) : value
                    });
                }
            });
        }
    ...
    this._getComments = function(task) {
        ...
        task.updates.forEach(function(update) {
            ...
            if (update.updateObjCode === "NOTE") {
                ...
                comments.push({
                    body: description + self._formatToJira(update.message),
    ...
    this._processTasks = function(pages) {
        pages.forEach(function(tasks, i) {
            tasks.forEach(function(task, n) {
                var issue = self._createIssueFromTask(task);
    ...
    this.feast = function() {
        ...
        return fs.writeFile("export.json", JSON.stringify(data));
    };
